    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Resumen de tu estudio contable</p>
        </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 mb-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-medium text-white">Obligaciones Próximas</h2>
          <span className="text-zinc-500 text-sm">Ver todas →</span>
        </div>
        <div className="space-y-3">
          {mockObligaciones.map((obligacion) => (
            <div
              key={obligacion.id}
              className="flex items-center justify-between p-4 rounded-lg bg-zinc-800/40 border border-zinc-700/50 hover:border-zinc-700 transition-colors"
            >
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-zinc-800">
          {filteredEmpresas.map((empresa) => (
            <div
              key={empresa.id}
              className="flex items-center justify-between p-4 hover:bg-zinc-800/40 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-white text-sm font-medium">{empresa.nombre}</p>
  ...
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors">
      ...
      <div className="flex items-center gap-1 mt-2">
        ...
        {trend === "warning" && <AlertTriangle className="h-3 w-3 text-yellow-400" />}
        <span className={cn(
          "text-xs",
          trend === "up" ? "text-green-400" :
          trend === "warning" ? "text-yellow-400" : "text-zinc-500"
        )}>{change}</span>
      </div>
    </div>
  );
